export const MARKDOWN_CHATS = [
  {
    role: "user",
    content: `Bagaimana cara mengajukan passport?`,
  },
  {
    role: "assistant",
    content: `# Cara mengurus passport

Untuk mengurus passport di Indonesia, ikuti langkah-langkah berikut:

## Persyaratan Dokumen
- KTP asli dan fotokopi
- Kartu Keluarga asli dan fotokopi
- Akta kelahiran asli dan fotokopi
- Pas foto berwarna ukuran 4x6 cm (2 lembar)

## Langkah Pengajuan
1. Daftar online di https://antrian.imigrasi.go.id
2. Pilih kantor imigrasi terdekat
3. Isi formulir permohonan
4. Upload dokumen persyaratan
5. Pilih jadwal kedatangan

## Proses di Kantor Imigrasi
1. Verifikasi dokumen
2. Pembayaran biaya passport
3. Foto dan sidik jari
4. Wawancara singkat
5. Tunggu proses pencetakan (3-5 hari kerja)

**Biaya:** Rp 350.000 untuk passport biasa (48 halaman)
`,
  },
  {
    role: "user",
    content: `Saya perlu legalisir dokumen untuk keperluan beasiswa. Bagaimana prosedurnya di kedutaan?`,
  },
  {
    role: "assistant",
    content: `# Legalisir Dokumen di Kedutaan

Untuk legalisir dokumen di kedutaan, berikut prosedur yang harus diikuti:

## Dokumen yang Diperlukan
- Dokumen asli yang akan dilegalisir
- Fotokopi dokumen tersebut
- Passport asli dan fotokopi
- Surat permohonan legalisir

## Langkah-langkah Pengajuan
1. **Persiapan dokumen** - Pastikan semua dokumen lengkap dan dalam kondisi baik
2. **Kunjungi kedutaan** - Datang pada jam operasional (biasanya 09:00-12:00)
3. **Ambil nomor antrian** - Daftar di loket informasi
4. **Serahkan dokumen** - Berikan semua persyaratan ke petugas
5. **Pembayaran** - Bayar biaya legalisir sesuai tarif

## Waktu Proses
- **Reguler:** 3-5 hari kerja
- **Express:** 1-2 hari kerja (biaya tambahan)
- **Same day:** Tersedia untuk kasus mendesak

## Biaya Legalisir
- Dokumen pendidikan: $25-50 per dokumen
- Dokumen sipil: $30-60 per dokumen
- Biaya tambahan untuk layanan express

**Catatan:** Jam operasional dapat berubah pada hari libur nasional
`,
  },
  {
    role: "user",
    content: `Bagaimana cara perpanjang SIM di Indonesia?`,
  },
  {
    role: "assistant",
    content: `# Perpanjangan SIM

Untuk memperpanjang Surat Izin Mengemudi (SIM) di Indonesia, ikuti langkah-langkah berikut:

## Dokumen yang Diperlukan
- SIM lama (yang masih berlaku/masa tenggang)
- KTP asli dan fotokopi
- Surat keterangan sehat jasmani & rohani (dari dokter/klinik)
- Pas foto terbaru (latar belakang sesuai ketentuan)

## Cara Perpanjang SIM
1. **Online** melalui aplikasi Digital Korlantas POLRI (Android/iOS)
   - Registrasi akun
   - Upload dokumen
   - Pilih jadwal & lokasi pengambilan SIM
   - Lakukan pembayaran online
2. **Offline** datang langsung ke Satpas atau layanan SIM Keliling

## Biaya Resmi
- SIM A: Rp 80.000
- SIM C: Rp 75.000

**Catatan:** Perpanjangan dapat dilakukan mulai 90 hari sebelum masa berlaku habis.
`,
  },
  {
    role: "user",
    content: `Saya mau daftar BPJS Kesehatan, bagaimana caranya?`,
  },
  {
    role: "assistant",
    content: `# Pendaftaran BPJS Kesehatan

## Persyaratan Dokumen
- KTP dan KK asli + fotokopi
- Buku tabungan (untuk autodebet)
- Pas foto ukuran 3x4 (1 lembar)

## Langkah Pendaftaran
1. **Online** lewat aplikasi Mobile JKN atau website https://daftar.bpjs-kesehatan.go.id
2. **Offline** di kantor BPJS Kesehatan terdekat
3. Isi formulir pendaftaran
4. Pilih kelas layanan (Kelas I, II, atau III)
5. Bayar iuran pertama di bank/merchant resmi
6. Kartu BPJS akan dicetak (fisik/digital)

## Iuran per Bulan (2025)
- Kelas I: Rp 150.000
- Kelas II: Rp 100.000
- Kelas III: Rp 35.000 (subsidi pemerintah)

**Tips:** Simpan bukti pembayaran iuran untuk mempermudah verifikasi.
`,
  },
  {
    role: "user",
    content: `Bagaimana prosedur membuat NPWP untuk karyawan baru?`,
  },
  {
    role: "assistant",
    content: `# Cara Membuat NPWP (Nomor Pokok Wajib Pajak)

## Dokumen yang Dibutuhkan
- KTP asli dan fotokopi
- Surat keterangan kerja dari perusahaan
- Fotokopi KK

## Cara Pengajuan
1. **Online**
   - Akses situs https://ereg.pajak.go.id
   - Daftar akun
   - Isi formulir pendaftaran
   - Upload dokumen persyaratan
   - Tunggu verifikasi (NPWP dikirim ke alamat terdaftar)
2. **Offline**
   - Datang ke Kantor Pelayanan Pajak (KPP) sesuai domisili
   - Ambil formulir pendaftaran
   - Serahkan dokumen
   - Petugas akan memproses NPWP

## Waktu Proses
- 1–3 hari kerja

**Biaya:** Gratis
`,
  },
];
